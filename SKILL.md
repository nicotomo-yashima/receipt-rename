...
name: receipt-rename
description: >
  ScanSnap等でスキャンしたレシート/領収書（PDF/JPG/PNG）のファイル名を、
  中身を読み取って「YYYYMMDD_店名_金額」形式に修正するスキル。
...
# Receipt Rename - レシートリネームスキル

スキャンしたレシート/領収書のファイル名を、実際の内容に基づいて修正する。
...
PDFファイルが含まれる場合のみ、`pdftoppm` の存在を確認する。なければ `brew install poppler` でインストール。
...
2. **画像読み取り**: Read ツールで変換後のPNG画像を読む

#### JPG/PNGの場合

1. **直接読み取り**: Read ツールでファイルをそのまま読む（変換不要）

#### 共通: 抽出する情報

Read ツールで画像を読み、以下を抽出:
- **日付**: レシートに印字された取引日（和暦の場合は西暦に変換）
- **店名**: 店舗名・屋号（ロゴ・ヘッダー・フッターから判別）
- **金額**: 合計金額（「合計」「合計金額」「領収金額」欄の値）

抽出後、現在のファイル名と読み取り結果を比較する。
...
| # | 旧ファイル名 | 新ファイル名 |
|---|---|---|
| 1 | 20260101_株式会社_(金額).pdf | 20250315_サイゼリヤ渋谷店_1200.pdf |
| 2 | IMG_2048.jpg | 20250320_スターバックス新宿店_500.jpg |
| ... | ... | ... |

### 元から正しかった（2件）
- 20250401_ローソン駅前店_350.pdf
...
- 画像が暗い・ぼやけている場合は解像度を `-r 300` に上げて再試行（PDF）
